...
**Codex generation run — ✅ executed by Codex 2026-07-07** (protocol: [codex-run.md](codex-run.md); results: [codex-run-results.md](codex-run-results.md) + snippets). **5/5 scenarios, every validator exit 0.** Highlights: scenario 2 (pressure) *beat* the recorded Claude column — its snippet compiled first-try because it used the numeric-slot `Stat.Summary` pattern folded into the skill after Claude's regression: the eval loop compounding across vendors. Scenario 5 clean (zero GGNet leakage in the Plotly answer). **Residual packaging note:** skill discovery was *manual* — Codex's session did not register `ggnet` from the in-repo `.codex-plugin/plugin.json` (repo presence ≠ plugin installation in Codex's harness; both manifests are in HEAD). Trigger-accuracy on Codex is therefore measured only for the model's judgment, not the harness's auto-loading; document an explicit install step (or AGENTS.md pointer) when Codex's plugin registration mechanics are confirmed. **With that caveat, Phase 1 exit criteria are met — 1.8 closed.** Still open: Phase 3 (demand-gated).

## Codex review (2026-07-07) — findings & resolutions

A Codex session reviewed the skill (first cross-vendor audit; it ran the consistency test and eval suite itself — the D8 dual-target premise held). Resolutions:

- **trend vs trend_over_time drift (valid, my authoring bug):** table now uses canonical names; `trend` added as an alias (defense in depth); new `SkillDocDriftEvals` gates the table against the config — every row a real leaf, every function term resolvable, ❌ markers matching `ggnet.supported`. The generated-once-then-hand-edited table was trust, not verification; now it's a gate.
- **Stale "MCP will later" wording (valid):** chart-selection.md header now says prefer the registered server, manual fallback otherwise.
- **Supported-without-recipe (framing rejected, substance accepted):** Tier C was a recorded decision, not an oversight — but the pressure-test regression proved unvalidated prose snippets are traps. Tier-C compositions are now real compiled+rendered code (`TierCCompositionTests`: dot plot, dumbbell, waffle — which also settled that categorical axes need struct types, documented in geoms.md), geoms.md carries the verbatim chains under a consistency rule, and the eval asserts supported ⇒ recipe ∨ note ∨ caveat.
- **Extract shared validation logic (rejected):** the script/tool duplication (~40 lines) is deliberate — the script's value is running with nothing but the dotnet CLI; coupling it to the server project to save forty lines inverts the trade. The real risk (silent drift) is covered by the accepted half: `ValidatorScriptEvals` smoke-tests the standalone script in CI, so both twins are exercised.
