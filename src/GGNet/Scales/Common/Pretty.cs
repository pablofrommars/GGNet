...
public static class Pretty
{
    ...
    private static (double lower, double upper, int n) PrettyBase(double lo, double up, int ndiv, int min_n, double shrink_sml, double h, double h5, int eps_correction)
    {
        ...
        if ((U = 2 * _base) - cell < h * (cell - unit))
        {
            ...
            {
                unit = U;
                if ((U = 10 * _base) - cell < h * (cell - unit))
                    unit = U;
            }
        }
    ...
    {
        var z = PrettyBase(min, max, n, (int)(5 / 3.0), shrink_sml, h, h5, eps_correction);
    ...
    public static double[]? Run(double min, double max, int n = 5)
    {
        var z = PrettyBase(min, max, n, (int)(5 / 3.0), shrink_sml, h, h5, eps_correction);
    ...
    public static ulong Diff(LocalDateTime min, LocalDateTime max)
    {
        var period = max - min;
    ...
    public static LocalDateTime[] Year(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
        var length = (int)Run(1.0, diff / (double)(3600 * 24 * 365), n)![1];
    ...
    private static readonly int[] length_months = [1, 2, 3, 4, 6, 12];

    public static LocalDateTime[] Month(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
    ...
    public static LocalDateTime[] Day(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
        var length = (int)Run(1.0, diff / (double)(3600 * 24), n)![1];
    ...
    public static LocalDateTime[] Hour(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
    ...
    private static readonly int[] length_minutes = [1, 2, 5, 10, 15, 30, 60];

    public static LocalDateTime[] Minute(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
        var span = diff / 60.0;
    ...
    private static readonly int[] length_secondes = [1, 2, 5, 10, 15, 30, 60];

    public static LocalDateTime[] Second(LocalDateTime min, LocalDateTime max, ulong diff, int n)
    {
        var length = 60;
        {
            var minfit = double.MaxValue;
    ...
    public static LocalDateTime[] Switch(LocalDateTime min, LocalDateTime max, int n = 5)
    {
        var diff = Diff(min, max);

        return (diff / (double)n) switch
        {
            double sec when sec > 3600 * 24 * 365 => Year(min, max, diff, n),
            double sec when sec > 3600 * 24 * 30 => Month(min, max, diff, n),
            double sec when sec > 3600 * 24 => Day(min, max, diff, n),
            double sec when sec > 3600 => Hour(min, max, diff, n),
            double sec when sec > 60 => Minute(min, max, diff, n),
            _ => Second(min, max, diff, n),
        };
    }
}
